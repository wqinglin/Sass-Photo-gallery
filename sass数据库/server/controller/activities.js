...
class activitiesController {
    ...
    static async add(ctx) {
        let data = await activitiesModel.findAll({})
        await ctx.render('admin/activities/add', { data: data })
    }
    static async doadd(ctx) {
        let params = ctx.req.body
        params.imgurl = ctx.req.file.filename;
        await activitiesModel.create(params)
        ctx.redirect('/admin/activities')
    }
    // static async doadd(ctx) {
    //     let params = ctx.req.body
    //     let arr = [];
    ...
    static async list(ctx) {
        ...
        let allResponse = await activitiesModel.findAll({ where})
        ...
        let response = await activitiesModel.findAll({
        ...
        await ctx.render('admin/activities/list', {data:response,totalPage,currentPage,next,prev,keyword})
    ...
    static async edit(ctx) {
        ...
        let dataone = await activitiesModel.findOne({where:{id}}) // 更新拿旧值
        console.log(dataone);
        await ctx.render('admin/activities/edit',{dataone:dataone,data:data})
    }
    static async doedit(ctx) {
        ...
        console.log(params);
       await activitiesModel.update(params, { where: { id } })
        ctx.redirect('/admin/activities');
    }
