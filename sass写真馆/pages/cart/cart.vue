<template>
	<view>
		...
		<!-- 占位 -->
		<u-notice-bar style="height: 40px;top: 0;" mode="horizontal"  type="error" :list="list"  ></u-notice-bar>
		<!-- 商品列表 -->
		...
		<zdy-tabbar :current-page="3" style="z-index: 999999;"></zdy-tabbar>
	</view>
	...
	export default {
		data() {
			return {
				...
				isStop: false,
				list: [
					"同时下多个订单，限同一人拍摄。使用前请及时联系工作人员预约拍摄时间，谢谢配合！"
				]
			}
		},
		onPageScroll(e) {
		...
		methods: {
			...
			joinGoods(goods) {
				...
				for (let i = 0; i < len; i++) {
					let row = this.cartlist[i];
					if (row.id == goods.id) { //找到商品一样的商品
						this.cartlist[i].number++; //数量自增
						isFind = true; //找到一样的商品
						break; //跳出循环
					}
				}
